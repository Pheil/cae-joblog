    ...
    self.port.once("userLogs",
        function chk_vs_logged_ews(theArray) {
            ...
            var ewsnumber2 = "EWS" + URLlocation.substr(numLen);
            ...
            if (assigned == 'Guy.') {
                ...
                div.setAttribute('style', 'position:absolute;top: 0;left: 0;right: 0;background:rgba(0, 0, 255, 0.5);margin: 0;height: 22px;text-align:center;border: 1px solid black;');
            } else if (assigned == 'Scott.') {
                var elmNewContent = document.createTextNode('CAE Job Log:  Assigned to Scott');
                div.setAttribute('style', 'position:absolute;top: 0;left: 0;right: 0;background:rgba(51, 204, 153, 0.5);margin: 0;height: 22px;text-align:center;border: 1px solid black;');
            } else if (assigned == 'Paul.') {
                var elmNewContent = document.createTextNode('CAE Job Log:  Assigned to Paul');
                div.setAttribute('style', 'position:absolute;top: 0;left: 0;right: 0;background:rgba(255, 153, 0, 0.5);margin: 0;height: 22px;text-align:center;border: 1px solid black;');
            } else if (assigned == 'Suzhou.') {
                var elmNewContent = document.createTextNode('CAE Job Log:  Assigned to Suzhou');
                div.setAttribute('style', 'position:absolute;top: 0;left: 0;right: 0;background:rgba(255, 255, 224, 0.5);margin: 0;height: 22px;text-align:center;border: 1px solid black;');
            } else {
                var elmNewContent = document.createTextNode('CAE Job Log:  Unassigned');
                div.setAttribute('style', 'position:absolute;top: 0;left: 0;right: 0;background:rgba(200, 54, 54, 0.5);margin: 0;height: 22px;text-align:center;border: 1px solid black;');
            }
